$(function () {
    ...
    $("#sure").click(function () {
        var txt = $("#page").val();
        if (txt != '') {
            window.location.href = '/market/sx_spot_checkac/index/parent_id/'+ $('.active').attr('data-pid') + '?page=' + txt;
        } else {
            window.location.href = '/market/sx_spot_checkac/index/parent_id/'+ $('.active').attr('data-pid');
        }
    });
    ...
    $("#btn_search").click(function () {
        ...
        if (txt != '') {
            window.location.href = '/market/sx_spot_checkac/index/parent_id/'+ $('.active').attr('data-pid') + '?key=' + txt;
        } else {
            window.location.href = '/market/sx_spot_checkac/index/parent_id/'+ $('.active').attr('data-pid');
        }
    });
    ...
    $("#btn_check").click(function (ev) {
        ...
        if (chk_value.length == 0) {
            ...
            $.post('/market/sx_spot_checkac/check',{id:chk_value.join(',')},function(data){
                   // console.log(data);
                if(data==1){
