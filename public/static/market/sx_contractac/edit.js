$(function () {
    ...
    $('.back-btn').on('click', function () {
     ...
     $("#merchant_ids").change(function(){

        var id=$(this).val();
  
        $.post('/market/sx_contractac/merchant_name',{id:id},function(data){
            var data=JSON.parse(data);

            $("#stall_number").html(data['stall_number']);
            $("#stall_ids").val(data['stall_id']);
        });
    });
    //有头像就显示删除按钮
    ...
    laydate({
        elem: '#property_start_time'
    });
    laydate({
        elem: '#property_end_time'
    });
    laydate({
