$(function(){
    ...
    $("#sure").click(function () {
        var txt = $("#page").val();
        if (txt != '') {
            window.location.href = '/market/sx_merchantac/index/parent_id/3?page=' + txt;
        } else {
            window.location.href = '/market/sx_merchantac/index/parent_id/3';
        }
    });

	//Excel导出
	$("#btn_export").click(function(){
		window.location.href = '/market/sx_merchantac/export';
	});

	//添加
	$('#btn_add').on('click',function () {
		location.href = '/market/sx_merchantac/add/parent_id/' + $('.active').attr('data-parent');
    });
    //修改
    ...
    $("#btn_edit").click(function () {
        ...
        if (chk_value.length == 0) {
            ...
            // alert($('#page').val());
            window.location.href = '/market/sx_merchantac/edit/page/' + $('#page').val() + "?id=" + chk_value[0];
        }
    });
        ...
        if (chk_value.length == 0) {
            ...
            $('.confirm').fadeIn(function () {
                ...
                $('#del-confirm').on('click', function () {
					$.post('/market/sx_merchantac/del',{id:chk_value.join(',')},function(data){
	...
	$("#query_btn").click(function(){
		...
		if(txt != ''){
			window.location.href = '/market/sx_merchantac/index/parent_id/3'+"?key="+txt;
		}else{
			window.location.href = '/market/sx_merchantac/index/parent_id/3';
		}
