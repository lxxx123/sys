$(function(){
    ...
    $("#sure").click(function () {
        var txt = $("#page").val();
        if (txt != '') {
            window.location.href = '/market/ycysac/index/parent_id/154?page=' + txt;
        } else {
            window.location.href = '/market/ycysac/index/parent_id/154';
        }
    });

	//Excel导出
	/*$("#btn_export").click(function(){
		window.location.href = '/market/merchantac/export';
	});*/

	//添加
	/*$('#btn_add').on('click',function () {
		location.href = '/market/merchantac/add';
    });*/
    //修改
    ...
    $("#btn_edit").click(function () {
        ...
        if (chk_value.length == 0) {
            ...
            // alert($('#page').val());
            window.location.href = '/market/ycysac/edit/page/' + $('#page').val() + "?id=" + chk_value[0];
        }
    });
        ...
        if (chk_value.length == 0) {
            ...
            $('.confirm').fadeIn(function () {
                ...
                $('#del-confirm').on('click', function () {
					$.post('/market/merchantac/del',{id:chk_value.join(',')},function(data){

						if(data==1){
							window.location.href = '/market/merchantac/index/parent_id/154';
						}else{
	...
	$("#query_btn").click(function(){
		...
		if(txt != ''){
			window.location.href = '/market/ycysac/index/parent_id/154'+"?key="+txt;
		}else{
			window.location.href = '/market/ycysac/index/parent_id/154';
		}
