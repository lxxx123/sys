$(function () {
    ...
    //有头像就显示删除按钮
    if ($('#preview-photo').data('has') == 1) {
        $('.input-photo .del-btn').fadeIn();
    }
    //有头像就显示删除按钮
    if ($('#preview-photo1').data('has') == 1) {
        $('.input-photo1 .del-btn1').fadeIn();
    }
    if ($('#preview-photo2').data('has') == 1) {
        $('.input-photo2 .del-btn2').fadeIn();
    }
    if ($('#preview-photo3').data('has') == 1) {
        $('.input-photo3 .del-btn3').fadeIn();
    }
    if ($('#preview-photo4').data('has') == 1) {
        $('.input-photo4 .del-btn4').fadeIn();
    }
    if ($('#preview-photo5').data('has') == 1) {
        $('.input-photo5 .del-btn5').fadeIn();
    }
    if ($('#preview-photo6').data('has') == 1) {
        $('.input-photo6 .del-btn6').fadeIn();
    }
    if ($('#preview-photo7').data('has') == 1) {
    ...
    $('#save-btn').on('click', function () {
        if ($('#clip-image').attr('src') === '') {
            ...
            canvas.toBlob(function (blob) {
                //获取文件扩展名
                var ext = blob.type.substr(blob.type.indexOf('/') + 1);
                //用FormData组织数据
                var formData = new FormData();
                formData.append('file', blob, 'filename.' + ext); //添加文件名
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.status) {
                            ...
                            $('#photo').val(data.url);
                            //设置预览头像路径
                            $('#preview-photo').attr('src', data.url);
 ...
 // ----------------------------
   //打开头像裁剪模态框
    $('.input-photo1').on('click', function () {
            ...
            canvas.toBlob(function (blob) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.status) {
                            //设置隐藏头像input的value
                            $('#zhifubao_img').val(data.url);
                            //设置预览头像路径
                            $('#preview-photo1').attr('src', data.url);
                            $('#preview-photo1').fadeIn();
                            //显示头像上的删除按钮
                            console.log($('.input-photo1 .del-btn1').fadeIn());
    ...
    //头像模块删除图片
    $('.input-photo1 .del-btn1').on('click', function (ev) {
        //阻止冒泡
        ev.stopPropagation();
        ...
        $('#zhifubao_img').val('');
        //清空预览头像路径并隐藏
        $('#preview-photo1').attr('src', '/static/image/input-image.png');
        //隐藏删除按钮
        $(this).fadeOut();
    });
    // ----------------------------------------
   //打开头像裁剪模态框
            ...
            canvas.toBlob(function (blob) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.status) {
                            //设置隐藏头像input的value
                            $('#weixin_img').val(data.url);
                            //设置预览头像路径
                            $('#preview-photo2').attr('src', data.url);
    ...
    $('.input-photo2 .del-btn2').on('click', function (ev) {
        ...
        $('#weixin_img').val('');
        //清空预览头像路径并隐藏
        $('#preview-photo2').attr('src', '/static/image/input-image.png');
        //隐藏删除按钮
        $(this).fadeOut();
    });
// -----------------------------------
   //打开头像裁剪模态框
            ...
            canvas.toBlob(function (blob) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.status) {
                            //设置隐藏头像input的value
                            $('#business_img').val(data.url);
                            //设置预览头像路径
                            $('#preview-photo3').attr('src', data.url);
    ...
    $('.input-photo3 .del-btn3').on('click', function (ev) {
        ...
        $('#business_img').val('');
        //清空预览头像路径并隐藏
        $('#preview-photo3').attr('src', '/static/image/input-image.png');
        //隐藏删除按钮
        $(this).fadeOut();
    });
// --------------------------------
 //打开头像裁剪模态框
            ...
            canvas.toBlob(function (blob) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.status) {
                            //设置隐藏头像input的value
                            $('#health_img').val(data.url);
                            //设置预览头像路径
                            $('#preview-photo4').attr('src', data.url);
    ...
    $('.input-photo4 .del-btn4').on('click', function (ev) {
        ...
        $('#health_img').val('');
        //清空预览头像路径并隐藏
        $('#preview-photo4').attr('src', '/static/image/input-image.png');
        //隐藏删除按钮
        $(this).fadeOut();
    });
// --------------------------------
 //打开头像裁剪模态框
            ...
            canvas.toBlob(function (blob) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.status) {
                            //设置隐藏头像input的value
                            $('#id_number_img').val(data.url);
                            //设置预览头像路径
                            $('#preview-photo5').attr('src', data.url);
    ...
    $('.input-photo5 .del-btn5').on('click', function (ev) {
        ...
        $('#id_number_img').val('');
        //清空预览头像路径并隐藏
        $('#preview-photo5').attr('src', '/static/image/input-image.png');
        //隐藏删除按钮
        $(this).fadeOut();
    });
...
// --------------------------------
 //打开头像裁剪模态框
            ...
            canvas.toBlob(function (blob) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.status) {
                            //设置隐藏头像input的value
                            $('#other_img').val(data.url);
                            //设置预览头像路径
                            $('#preview-photo6').attr('src', data.url);
    ...
    $('.input-photo6 .del-btn6').on('click', function (ev) {
        ...
        $('#other_img').val('');
        //清空预览头像路径并隐藏
        $('#preview-photo6').attr('src', '/static/image/input-image.png');
        //隐藏删除按钮
        $(this).fadeOut();
    });
    // ----------------------------------------
   //打开头像裁剪模态框
            ...
            canvas.toBlob(function (blob) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.status) {
                            //设置隐藏头像input的value
                            $('#zjnx_img').val(data.url);
                            //设置预览头像路径
                            $('#preview-photo7').attr('src', data.url);
    ...
    $('.input-photo7 .del-btn7').on('click', function (ev) {
        ...
        $('#zjnx_img').val('');
        //清空预览头像路径并隐藏
        $('#preview-photo7').attr('src', '/static/image/input-image.png');
        //隐藏删除按钮
        $(this).fadeOut();
    });
