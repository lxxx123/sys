$(function () {
    ...
    $('#btn_add').on('click',function (ev) {
        ...
        $.post('/market/merchant_goodsac/add_post',data,function (data) {
    ...
    $('.operate').delegate('.btn_edit','click',function (ev) {
       ...
       var data = {
           ...
           unit_name:$(this).parents('.edit').find('.unit_id option:selected').text(),
       }
        $.post('/market/merchant_goodsac/edit_post',data,function (data) {
            data = JSON.parse(data);
            if(data.status){
                location.reload();
            }
    ...
    $('.operate').delegate('.btn_del','click',function (ev) {
        ...
        window.parent.$('.confirm').fadeIn(function () {
            ...
            window.parent.$('#del-confirm').on('click', function () {
                ...
                $.post('/market/merchant_goodsac/del',data,function (data) {
                    data = JSON.parse(data);
                    if(data.status){
                        location.reload();
                    }
