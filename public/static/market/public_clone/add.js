$(function () {
    ...
    // });
      $("#submit").on('click',function(){
        var chk_value = [];
        $('input[name="stall_list_id"]:checked').each(function () {
        ...
        if (chk_value.length == 0) {
          ...
          $.post('/market/public_cloneac/add_post',{stall_list:chk_value.join(','),form:JSON.stringify(d)},function(data){

                if(data.status==1){
                    window.location.href = '/market/public_cloneac/index/parent_id/'+data.parent_id;
                }
           });    
    ...
    $('#check-all').on('change', function () {
        ...
        $('input[name="stall_list_id"]').each(function () {
