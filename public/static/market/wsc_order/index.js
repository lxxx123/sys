$(function(){
    function query(date, more, page) {
        ...
        var url = '/market/wsc_orderac/index/parent_id/' + $('.active').attr('data-pid');
        if ($.param(data) !== '') {
            url = url + '?' + param;
        }
        location.href = url;
    ...
    $("#sure").click(function () {
        var txt = $("#page").val();
        if (txt != '') {
            window.location.href = '/market/wsc_orderac/index/parent_id/' + $('.active').attr('data-pid') + '?page=' + txt;
        } else {
            window.location.href = '/market/wsc_orderac/index/parent_id/' + $('.active').attr('data-pid');
        }
    });
